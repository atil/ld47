...
public class PlayerMotor : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        var isGrounded = IsGrounded(out Vector3 groundNormal, out Transform movingPlatform);
    ...
    private Vector3 ResolveCollisions(ref Vector3 playerVelocity)
    {
        ...
        for (var i = 0; i < _overlappingColliders.Length; i++)
        {
                ...
                out collisionNormal, out collisionDistance))
            {
                ...
                if (envColl.transform.TryGetComponent<Lerper>(out Lerper lerper) 
                    && lerper.IsPlaying
    ...
    public bool IsGrounded(out Vector3 groundNormal, out Transform movingPlatform)
    {
        ...
        foreach (var t in _groundedRayPositions)
        {
            ...
            {
                if (hit.transform.TryGetComponent<MovingPlatform>(out _))
                {
